...
export default function CartPage() {
  const { cartItems, totals, addToCart, updateCartItem, fetchCart } = useCart();
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
          ...
          <div className="flex flex-col gap-3">
            {cartItems.map((item) => (
              <div
                key={item.product_id}
                className={`flex gap-4 rounded-2xl border border-sage bg-white p-4 transition-opacity ${
                  actionLoading[item.product_id] === "remove" ? "opacity-40" : ""
                }`}
              >
                {/* Thumbnail */}
                <div
                  className="h-22 w-22 shrink-0 overflow-hidden rounded-xl"
                  style={{
                ...
                <div className="flex flex-1 flex-col justify-between">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-heading font-bold text-forest leading-snug">{item.product_name}</h3>
          ...
          {/* ── Order Summary ── */}
          <div className="h-fit rounded-2xl border border-sage bg-white p-6 lg:sticky lg:top-24">
            ...
            <button className="mt-5 w-full rounded-full bg-emerald py-3 font-heading text-sm font-semibold text-white transition-colors hover:bg-emerald-dark">
              Place order
            </button>
