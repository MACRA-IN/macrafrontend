...
import { Calendar, Clock, Pause, ArrowRight, ChefHat, LogOut } from "lucide-react";
...
export default function Dashboard() {
  ...
  const firstSlot = Array.isArray(deliverySlots) && deliverySlots.length > 0
    ? Object.values(deliverySlots[0])[0]
    : "12–2 PM";

  const progressPct = subscription.total_delivery_days > 0
    ? ((subscription.total_delivery_days - subscription.remaining_delivery_days) / subscription.total_delivery_days) * 100
    : 0;
      ...
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="mt-5 overflow-hidden rounded-3xl bg-gradient-to-br from-forest to-[#0a1f12] text-white">
          <div className="p-5 sm:p-6">
            ...
            {/* Progress */}
            <div className="mt-5">
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.4)" }}>
                <span>Progress</span>
                <span>{Math.round(progressPct)}% complete</span>
              </div>
              <div className="mt-1.5 h-1.5 overflow-hidden rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }}>
                <div className="h-full rounded-full bg-emerald transition-all duration-700" style={{ width: `${progressPct}%` }} />
              </div>
            </div>

            {/* Stats */}
            <div className="mt-4 grid grid-cols-3 gap-2.5">
              <StatCard icon={Calendar} label="Days left" value={subscription.remaining_delivery_days} sub={`of ${subscription.total_delivery_days}`} highlight />
              <StatCard icon={Clock} label="Next delivery" value={nextDeliveryLabel} sub={firstSlot} />
              <StatCard icon={Pause} label="Pause days" value={subscription.pause_days_left} sub="available" highlight />
            </div>
        ...
        <div className="mt-5 rounded-3xl border border-sage bg-white p-5 shadow-card sm:p-6">
          {subscription.slots && subscription.slots.length > 0 ? (
            <DeliveredSlots slots={subscription.slots} />
          ) : (
...
function StatCard({ icon: Icon, label, value, sub, highlight }) {
  return (
    <div className="rounded-2xl p-3.5" style={{ backgroundColor: "rgba(255,255,255,0.06)" }}>
      ...
      <p className={`mt-1.5 font-heading text-xl font-bold leading-none ${highlight ? "text-emerald" : "text-white"}`}>
        {value}
      </p>
      <p className="mt-0.5 text-[11px]" style={{ color: "rgba(255,255,255,0.4)" }}>{sub}</p>
    </div>
  );
}

function DeliveredSlots({ slots }) {
        ...
        {slots.map((slot, i) => (
          <div
            ...
            className={`flex items-center justify-between rounded-xl border px-3 py-2.5 ${
              slot.status === "delivered" ? "border-emerald/20 bg-emerald/5" : "border-sage"
            }`}
          >
            <div>
              <p className="font-heading text-sm font-semibold text-forest">{slot.product_name}</p>
              <p className="mt-0.5 text-xs text-text-muted">
                {new Date(slot.delivery_date).toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short" })} · {slot.slot}
              </p>
            </div>
            <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
              slot.status === "delivered" ? "bg-emerald/10 text-emerald" : "bg-sage/40 text-text-muted"
            }`}>
              {slot.status}
            </span>
          </div>
